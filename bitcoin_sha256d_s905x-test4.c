...
static void run_benchmark(uint64_t iterations, int num_threads, int core_offset, int pin_enabled, int spin_core) {
    ...
    if (spin_core >= 0) {
        printf("Busy Neighbor     : enabled (dummy spin thread pinned to core %d)\n", spin_core);
    }
    ...
    spin_thread_arg_t spin_arg = { spin_core, &spin_stop };
    int spin_started = 0;
    if (spin_core >= 0) {
...
int main(int argc, char *argv[]) {
    ...
    if (do_benchmark) {
        run_benchmark(iterations, num_threads, core_offset, pin_enabled, spin_core);
    }
