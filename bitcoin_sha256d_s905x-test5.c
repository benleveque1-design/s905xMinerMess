...
/* ----------------------------------------------------------------------
 * Diagnostic: a "busy neighbor" thread that pins to one core and burns
 * cycles with NO memory traffic and NO crypto instructions - just an
 * incrementing local counter. Used to isolate whether a slowdown seen
 * when adding a 4th hash worker comes from something SHA256-specific
 * (shared crypto/execution-unit pressure) versus generic contention
 * (power/DVFS budget sharing, shared L2/interconnect traffic, OS
 * scheduling pressure) that any busy 4th core would cause.
 *
 * duty_percent controls what fraction of each ~20ms cycle the thread
 * spends actually spinning vs. sleeping. Sweeping this from 100 down to
 * 0 traces out the shape of the contention: a roughly linear relationship
 * between duty cycle and hash-rate loss on the other 3 cores points to a
 * shared, gradually-divisible resource (e.g. a shared power/current
 * budget); a sharp cliff at some threshold points to a saturating
 * resource (e.g. a shared queue/arbiter that's fine until it's full).
 * ---------------------------------------------------------------------- */
...
typedef struct {
    ...
    int duty_percent; /* 0-100: 100 = always spinning (old behavior), 0 = always idle */
} spin_thread_arg_t;
...
static void *spin_worker(void *arg) {
    ...
    /* volatile so the compiler can't optimize the loop away; this is pure
     * register/ALU work with no loads/stores beyond the local variable,
     * so it exercises core occupancy and power draw without touching the
     * cache hierarchy or memory bus. */
    ...
    while (!atomic_load(targ->stop_flag)) {
        if (busy_ns > 0) {
            ...
            do {
                /* amortize the clock_gettime cost over a batch of adds so
                 * the "busy" portion stays ALU-bound rather than becoming
                 * a timer-syscall loop */
...
static void run_benchmark(uint64_t iterations, int num_threads, int core_offset, int pin_enabled, int spin_core, int spin_duty) {
    ...
    if (spin_core >= 0) {
        printf("Busy Neighbor     : enabled (dummy spin thread pinned to core %d, %d%% duty cycle)\n", spin_core, spin_duty);
    }
    ...
    spin_thread_arg_t spin_arg = { spin_core, &spin_stop, spin_duty };
    int spin_started = 0;
    if (spin_core >= 0) {
...
int main(int argc, char *argv[]) {
    ...
    if (do_benchmark) {
        run_benchmark(iterations, num_threads, core_offset, pin_enabled, spin_core, spin_duty);
    }
