 ...
 * ============================================================================
 *
 * This models the target architecture: 3 pinned hash workers that only ever
 * touch a shared "current job" through a rarely-checked atomic version
 * number (no locking on the hot hashing path), plus a dedicated control
 * thread that owns everything that ISN'T hashing - job polling, share
 * submission, and periodic reconnects - talking over a real loopback TCP
 * socket so the I/O costs (syscalls, context switches, kernel copies) are
 * genuine rather than synthetic.
 *
 * A tiny "stub pool" thread stands in for a remote Stratum pool. It is
 * deliberately left UNPINNED (and is mostly blocked in poll()/accept(),
 * so it costs almost nothing) because in a real deployment the pool is a
 * remote server, not local CPU competing with your hash cores - pinning
 * it to core 3 would double-count work that would never actually be
 * local. Only the control_worker thread (the real "mining client" side)
 * is pinned to core 3, matching the requested architecture.
 * ---------------------------------------------------------------------- */
...
/* Called only by the control thread (core 3), roughly a few times a
 * second - the lock is held for a 80-byte memcpy, negligible contention. */
...
/* Called by hash workers only when their cached version is stale (every
 * ~8192 nonces, i.e. a handful of times per second per core at these
 * hashrates) - this is the only shared-state touch on the hot path. */
...
/* Called from hash workers when a candidate clears the synthetic share
 * target - rare by construction (see --share-bits), so the brief lock is
...
static void *stub_pool_thread(void *arg) {
    ...
    while (!atomic_load(a->stop_flag)) {
        ...
        while (!atomic_load(a->stop_flag) && conn_fd < 0) {
            ...
            int pr = poll(&pfd, 1, 100); /* 100ms so we notice stop_flag promptly */
            if (pr > 0 && (pfd.revents & POLLIN)) {
                conn_fd = accept(listen_fd, NULL, NULL);
            }
        }
        if (conn_fd < 0) break; /* stopping */
        ...
        while (!atomic_load(a->stop_flag)) {
            ...
            if (n <= 0) break; /* client reconnecting - go back to accept() */
            buf[n] = '\0';
...
typedef struct {
    ...
    int stub_port;
    int job_interval_ms;
    int reconnect_interval_ms; /* 0 = never simulate a reconnect */
...
static int control_connect(int port) {
    ...
    /* Short recv timeout so the control loop can keep checking stop_flag
     * and its timers instead of blocking indefinitely - this is the
     * thread's only source of "CPU work": mostly parked in recv()/poll(),
     * exactly like a real Stratum client thread waiting on the pool. */
    struct timeval tv;
    tv.tv_sec = 0;
    tv.tv_usec = 150000;
...
static void *control_worker(void *arg) {
    ...
    while (!atomic_load(targ->stop_flag)) {
        ...
        /* Poll for a new job at the configured cadence - simulates
         * mining.notify / job refresh traffic. */
        if (fd >= 0 && timespec_diff_ns(&now, &last_job) / 1000000 >= targ->job_interval_ms) {
            ...
            if (send(fd, req, strlen(req), 0) >= 0) {
                ...
                if (n > 0) {
                    ...
                    /* Simulate a changed extranonce/merkle root by
                     * perturbing the timestamp field with the job
                     * counter - the exact bytes touched don't matter for
                     * this benchmark, only that a real 80-byte header
                     * gets republished for hash workers to pick up. */
        ...
        /* Drain any shares the hash workers found and "submit" them,
         * with a real round-trip over the loopback socket. */
        share_t s;
        while (fd >= 0 && share_queue_pop(targ->shares, &s)) {
            ...
            if (send(fd, msg, (size_t)len, 0) >= 0) {
                char ack[16];
                recv(fd, ack, sizeof(ack), 0); /* best-effort; timeout-bounded */
        ...
        if (targ->reconnect_interval_ms > 0 &&
            ...
            if (fd >= 0) close(fd);
            usleep(20000); /* brief backoff, like a real reconnect delay */
            fd = control_connect(targ->stub_port);
            atomic_fetch_add(&targ->reconnects, 1);
            last_reconnect = now;
        }

        /* Idle tick: this thread is I/O-bound, not CPU-bound, so it
         * spends nearly all its time here or blocked in recv()/poll(),
         * not spinning - the opposite character from the earlier
         * synthetic 100%-duty spin tests. */
        usleep(20000);
    }
...
/* Each worker hashes its own disjoint slice of the nonce space. In legacy
 * mode (job == NULL) it uses a single fixed header template, matching the
 * original benchmark exactly - zero shared state, zero behavior change.
 * In control-plane mode (job != NULL) it instead pulls its header from a
 * shared_job_t, re-checking a cheap atomic version number every ~8192
 * nonces (a few times a second per core) rather than on every hash, and
 * reports any share-target hits to a share_queue_t for the control
 * thread to "submit". This is the only shared-state touch on the hot
 * hashing path. */
typedef struct {
    ...
    shared_job_t  *job;            /* NULL = legacy fixed-header mode */
    share_queue_t *shares;         /* NULL = don't check/report shares */
    int            share_zero_bits; /* synthetic share-target strictness */
} bench_thread_arg_t;
...
static void *bench_worker(void *arg) {
    ...
    if (targ->job == NULL) {
        /* ---- Legacy path: unchanged from the original benchmark ---- */
        const char *b125552_hex = "0100000081cd02ab7e569e8bcd9317e2fe99f2de44d49ab2b8851ba4a308000000000000e320b6c2fffc8d750423db8b1eb942ae710e951ed797f7affc8892b0f1fc122bc7f5d74df2b9441a42a14695";
        ...
        for (uint64_t i = 0; i < targ->count; i++) {
            ...
            update_header_nonce_le(local_header, nonce);
...
/* Best-effort sysfs readers for thermal/frequency snapshots around a
 * benchmark run - lets a run self-report enough context to catch thermal-
 * history confounds (e.g. a run that benefits from starting cooler than
 * the one before it) without needing a second monitoring session. Returns
 * -1 on any failure (missing path, permissions, non-Linux, etc.) so
 * callers can just skip printing the line rather than erroring out. */
...
static void run_benchmark(uint64_t iterations, int num_threads, int core_offset, int pin_enabled,
    ...
    /* Control-plane infrastructure: only stood up when -c is used. */
    shared_job_t job;
    share_queue_t shares;
    ...
    if (control_core >= 0) {
        ...
        if (pthread_create(&stub_thread_handle, NULL, stub_pool_thread, &stub_arg) == 0) {
            stub_started = 1;
            /* Wait briefly for the stub pool to start listening and hand
             * us its ephemeral port before starting the control thread. */
     ...
     * (Mutually exclusive with -c at the CLI level, enforced in main().) */
...
int main(int argc, char *argv[]) {
    ...
    if (spin_core >= 0 && control_core >= 0) {
        fprintf(stderr, "Error: -x/--spin-core and -c/--control-core are mutually exclusive "
                         "(synthetic busy-neighbor test vs. real control-plane architecture).\n");
        return 1;
    }
